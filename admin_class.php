...
Class Action {
	...
	function save_faculty(){
		...
		$check = $this->db->query("SELECT * FROM faculty where email ='$email' ".(!empty($id) ? " and id != {$id} " : ''))->num_rows;
		if($check > 0){
			return 2;
			exit;
		}
	...
	function save_class_subject(){
		...
		$check = $this->db->query("SELECT * FROM class_subject where subject_id ='$subject_id' ".(!empty($id) ? " and id != {$id} " : ''))->num_rows;
		if($check > 0){
			return 2;
			exit;
		}
	...
	function get_att_record(){
		...
		$qry = $this->db->query("SELECT co.course,concat(c.level) as `class` FROM class_subject cs inner join class c on c.id = cs.class_id inner join courses co on co.id = cs.subject_id where cs.id = {$class_subject_id} ");
