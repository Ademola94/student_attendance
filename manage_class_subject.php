    ...
    <form action="" id="manage-class_subject">
        ...
        <div class="form-group">
            ...
            <select name="class_id" id="" class="custom-select select2">
                <option value=""></option>
                <?php
                $class = $conn->query("SELECT c.*,concat(c.level) as `class` FROM `class` c inner join courses co on co.id = c.course_id order by concat(co.course,' ',c.level,'-',c.section) asc");
        ...
        <div class="form-group">
            <label for="" class="control-label">Choose Lecturer</label>
        ...
        <div class="form-group">
            ...
            <select name="subject_id" id="" class="custom-select select2">
                ...
                while($row=$class->fetch_assoc()):
                ?>
                <option value="<?php echo $row['id'] ?>" <?php echo isset($subject_id) && $subject_id == $row['id'] ? 'selected' : '' ?>><?php echo ucwords($row['course']) ?></option>
                <?php endwhile; ?>
            </select>
        </div>
    </form>
    ...
    $('#manage-class_subject').submit(function(e){
        ...
        $.ajax({
            ...
            success:function(resp){
                ...
                $('#msg').html('<div class="alert alert-danger mx-2">Course Already Assign.</div>')
                end_load()
                }   
            }
        })
